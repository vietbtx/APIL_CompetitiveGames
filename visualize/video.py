...
N_GAMES = 1000 if not args.video else 4  # for evaluating
...
video_params = {
    'save_dir': "saved_videos",                 # directory to store videos in.
...
def main():
    ...
    env = AnnotatedGymCompete(env, args.env_name, None, None, None, None, None,
                                              **video_params['annotation_params'])
    env = VideoWrapper(env, video_params['save_dir'], video_params['single_file'])
    ...
    if args.use_retrained_vic:
        ...
        vic_path = f"{vic_log_dir}/vic_policy.pt" if len(args.retrained_vic_path)==0 else args.retrained_vic_path
    ...
    logs = {}
    vic_tsne_acts = []
    # video_recorder = VideoRecorder(env, f"videotest.mp4", enabled=True) if args.video else None
