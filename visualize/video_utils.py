...
import mujoco_py_131 as mujoco_py
...
class AnnotatedGymCompete(gym.Wrapper):
    ...
    def __init__(self, env, env_name, vic_name, adv_name, mask_agent_index, resolution, font, font_size, short_labels, camera_config,
                 ypos=0.0, spacing=0.05, num_frames=120, draw=True):
        super(AnnotatedGymCompete, self).__init__(env)

        # Set agent colors
        self.env_name = env_name
        self.victim_index = 0 if "YouShallNotPassHumans" not in env_name else 1
        self.mask_agent_index = mask_agent_index
        self.agent_mapping = {
            0: (0 == self.victim_index, 0 == self.mask_agent_index, None, None),
            1: (1 == self.victim_index, 1 == self.mask_agent_index, None, None),
        }
        self.vic_name = vic_name
        self.adv_name = adv_name

        # Text overlay
        self.font = ImageFont.truetype('visualize/arial.ttf', font_size)
        self.font_bold = ImageFont.truetype('visualize/arial.ttf', font_size)
    ...
    def step(self, vic_action, adv_action):
        return self._step(vic_action, adv_action)

    def render(self, mode='human'):
        return self._render(mode)

    def _step(self, vic_action, adv_action):
        obs, rew, done, info = self.env.step(vic_action, adv_action)
        if done:
            ...
            if winner is None:
                ...
                if "YouShallNotPassHumans" in self.env_name:
                    winner = 1 - winner
                k = f'win{winner}'
            self.result[k] += 1
        ...
        if mode == 'rgb_array':
            ...
            for k, label in texts.items():
                ...
                if cur_agent is not None:
                    header = self.vic_name if label == "Victim" else self.adv_name
                _label = "Adversary" if label == 'Opponent' else label
                scores = f"{_label} = {self.result[k]}"
